...
export class TabsSortingCommand implements Command<string> {
  ...
  constructor(
    public sortByUrl: boolean) {
  }

  async execute(): Promise<ExecutionResult<string>> {
    if (this.sortByUrl) {
      const currentTabs = await chrome.tabs.query({})
      this.tabsMap = new Map(currentTabs
      ...
      const sortedMap = new Map([...this.tabsMap.entries()]
        .sort((a, b) =>
          a[1].url?.localeCompare(b[1].url)))
      await chrome.tabs.move(Array.from(sortedMap.keys()), { index: -1 })
      return Promise.resolve(new ExecutionResult('', 'tabs sorted by URL',
